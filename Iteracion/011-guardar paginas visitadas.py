...
# timeout + busy_timeout para minimizar "database is locked"
conn = sqlite3.connect(db_path, timeout=30)
conn.execute("PRAGMA busy_timeout = 30000;")
conn.execute("PRAGMA journal_mode = WAL;")
cur = conn.cursor()

# Tabla docs (como antes)
cur.execute("""
...
# =========================
# REANUDAR: cargar visited desde SQLite
# =========================
visited_pages = set()
...
# La cola arranca con la start url, pero normalizada
start_norm = normalize_url(START_URL)
queue = deque([start_norm])

base_netloc = urlparse(start_norm).netloc
...
while queue and len(visited_pages) < MAX_PAGES:
    ...
    # visited aquí significa "ya existe en tabla pages" o "ya lo visitamos en esta ejecución"
    if page_url in visited_pages:
        continue

    print(f"\n[{len(visited_pages)+1}/{MAX_PAGES}] Visitando: {page_url}")
    ...
    # Guardar/actualizar la página en SQLite (pages)
    title = ""
